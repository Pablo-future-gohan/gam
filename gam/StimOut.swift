...
class StimOut: SKScene, SKPhysicsContactDelegate {
   ...
   func makeRowOfBlocks(_ numRow: Int, _ SpawnAt: Int) {
       for i in 0...9 {
           let hue = (CGFloat.random(in: 0.5...0.58) + CGFloat(numRow) / 18.0)
           ...
           block = SKSpriteNode(
               color: UIColor(
                   hue: hue,
                   saturation: 1,
                   brightness: 0.2 + CGFloat(numRow) / 10.0,
                   alpha: 1
               ),
